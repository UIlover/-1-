...
############################################## 读MAP包 ###########################################
Map_file_Path = Path(os.getcwd()+"\Map.tt")
Mapbao_Path = Path("Map")
MuLu_CunZai=Mapbao_Path.is_dir()
while(not MuLu_CunZai):
    if(not Map_file_Path.is_file()):
        ...
        break
print('开始解压缩')
JieYa = py7zr.SevenZipFile(Map_file_Path,'r')
JieYa.extractall(os.getcwd()+"\Map")
JieYa.close()
print("Map目录存在")
map_id=0
Map_path=''
...
def GetGameMapId():
    ...
    Map_path = os.getcwd()+"\\Map\\"+str(map_id)+".png"
